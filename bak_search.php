...
elseif ($act == 'search')
{
    ...
    $where     = ' WHERE o.user_id=u.user_id AND o.platform=r.role_id ';
        
    if (strpos($keyword, '-') === false)
    {
        if($condition == 'tracking_sn' || $condition == 'order_sn')
        {
            $sql        = 'SELECT user_id FROM '.$ecs->table('order_info');
            $sql_append = " WHERE $condition='$keyword' LIMIT 1";
            
            $user_old = $db->getOne($sql.$sql_append);

            if (!$user_old) {
                $sql = 'SELECT user_id FROM '.$ecs->table('ordersyn_info');
                $user_new = $db->getOne($sql.$sql_append);
            }
            $where .= " AND o.$condition='$keyword' ";
        }
        else if ($condition == 'card_number')
        {
            $sql = 'SELECT user_id '.$ecs->table('memship_number')
            ." WHERE c.$condition=$keyword";

            $user_old = $db->getOne($sql.$sql_append);
            $where .= " AND c.$condition=$keyword ";
        }
        else if ($condition == 'user_id')
        {
            $sql_append = " AND u.user_id={$keyword} ";
            $sql        = 'SELECT user_id FROM '.$ecs->table('users').' u WHERE 1 '; 
            $user_old   = $db->getOne($sql.$sql_append);

            $sql      = 'SELECT user_id FROM '.$ecs->table('userssyn').' u WHERE 1 '; 
            $user_new = $db->getOne($sql.$sql_append);
        ...
        {
            $sql = 'SELECT user_id FROM '.$ecs->table('users').' u WHERE 1 ';
            $sql_append = " AND u.$condition LIKE '%$keyword%'";

            $user_old = $db->getAll($sql.$sql_append);

            $sql = 'SELECT user_id FROM '.$ecs->table('userssyn').' u WHERE 1 ';
            $user_new = $db->getAll($sql.$sql_append);
        ...
        $order_by       = ' ORDER BY o.confirm_time DESC ';
        $res_order_user = $res_no_order_user = $temp = array();
        ...
        if($user_old)
        {
            ...
            $sql_order      = 'SELECT user_id FROM '.$ecs->table('order_info')." WHERE user_id IN($user_id_list) GROUP BY user_id";
            $purchased_user = $db->getCol($sql_order);

            //没有订单的
            $no_purchased_user = array_diff($user_id,$purchased_user);
            if($purchased_user){
                $sql_select = $sql.' FROM '.$ecs->table('users')
                .' u LEFT JOIN '.$ecs->table('memship_number').' c ON u.user_id=c.user_id AND c.user_id<>0, '
                .$ecs->table('order_info').' o, '
                .$ecs->table('role').' r ';

                $sql_select .= $where.$order_by;
                $res_order_user = $db->getAll($sql_select);       
            }


            if($no_purchased_user){
                $no_purchased_user = implode(',',$no_purchased_user);
                $sql_select = "SELECT user_name,u.user_id,$tel admin_name AS add_admin,rank_points FROM ".$ecs->table('users').' u LEFT JOIN '.$ecs->table('memship_number')
                ." m ON m.user_id=u.user_id AND m.user_id<>0 WHERE u.user_id IN($no_purchased_user)";
            $res_no_order_user = $db->getAll($sql_select); //老顾客
            }

            $old_user_info = array_merge($res_order_user,$res_no_order_user);
            foreach($old_user_info as $val){
                if(!$temp[$val['user_id']]){
        ...
        if($user_new)
        {
            ...
            // 是否有订单
            $sql_order = 'SELECT user_id FROM '.$ecs->table('ordersyn_info')." WHERE user_id IN($user_id_list) GROUP BY user_id ";
            ...
            if ($purchased_user) {
                $sql_select = $sql.' FROM '.$ecs->table('userssyn')
                    .' u, '.$ecs->table('ordersyn_info') .' o, '
                    .$ecs->table('role').' r, '.$ecs->table('memship_number')
                    .' c ';
            ...
            if($no_purchased_user){
                $sql_select = "SELECT user_name,u.user_id,$tel admin_name AS add_admin,rank_points FROM ".$ecs->table('userssyn')." u WHERE user_id IN($user_id_list)";
            $res_no_order_user = $db->getAll($sql_select); //新顾客
            }

            $new_user_info = array_merge($res_order_user,$res_no_order_user);
            foreach($new_user_info as $val){
                if(!$temp[$val['user_id']]){
                    $temp[$val['user_id']] = $val;
                }
            }
            ...
            if($type)
            {
                ...
                if($res){
                    foreach ($res as &$val)
                    {
                        ...
                        $val['order_status'] = $val['order_status'].$val['shipping_status'].$val['pay_status'];
                        ...
                        if($val['order_status'] == '-200'){
                            $sql_select = 'SELECT user_name FROM '.$ecs->table('admin_user')
