...
elseif ($_REQUEST['act'] == 'history_order')
{
    ...
    $smarty->assign('platform_list',$platform_list);
    $smarty->assign('num', sprintf('（共%d条）[测试阶段]', $order_list['record_count']));
...
elseif ($_REQUEST['act'] == 'order_detail')
{
    ...
    if (isset($_REQUEST['current_order'])||isset($_REQUEST['abnormal_list'])||isset($_REQUEST['returned_list'])||isset($_REQUEST['canceled_list'])||isset($_REQUEST['history_order'])||isset($_REQUEST['finished_order'])||isset($_REQUEST['everyday_order_check'])) {
        ...
        $sql_select = 'SELECT order_lock,lock_timeout,admin_id FROM '.$GLOBALS['ecs']->table($order_table_name)." WHERE order_id=$id";
        ...
        if (!admin_priv('all', '', false) && !admin_priv('order_group_view', '', false)) {
            ...
            if ($lock_status['order_lock'] != $_SESSION['admin_id'] && $lock_status['admin_id'] != $_SESSION['admin_id']) {
                $sql_select = 'SELECT user_name FROM '.$GLOBALS['ecs']->table('admin_user')." WHERE user_id={$lock_status['order_lock']}";
    ...
    // 月末禁止关闭订单
    if (date('d') > 27 && $_SESSION['role_id'] != 13 && $_SESSION['admin_id'] != 4) {
    ...
    if (!empty($order_info)) {
        ...
        $sql_select = 'SELECT user_name, user_id FROM '.$GLOBALS['ecs']->table('admin_user').
            ' WHERE action_list LIKE "%,order_synchro,%" AND status>0';
        $admin_list_order = $GLOBALS['db']->getAll($sql_select);
        $smarty->assign('admin_list_order', $admin_list_order);

        // 顾客归属
        $sql_select = 'SELECT user_name, user_id FROM '.$GLOBALS['ecs']->table('admin_user').
            ' WHERE action_list LIKE "%,user_synchro,%" AND status>0';
    ...
    //$now_time = time();
      ...
      */
    admin_log("订单ID是$id",'view','order_detail','order',$id);  
    die($json->encode($res));
}
...
elseif ($_REQUEST['act'] == 'ordersyn_verify')
{
    ...
    // 验证该订单是否已经被锁定
    $now_time = time();
    $sql_select = 'SELECT order_lock FROM '.$GLOBALS['ecs']->table('ordersyn_info')." WHERE order_id=$order_id AND lock_timeout>$now_time";
    ...
    if (isset($_REQUEST['user_admin_id'])) {
        ...
        // 将顾客从临时顾客表转存至正式顾客表中
        $sql_insert = 'INSERT INTO '.$GLOBALS['ecs']->table('users').'(user_name,add_time,aliww,qq,home_phone,'.
            'mobile_phone,team,role_id,platform,admin_id,admin_name,customer_type) SELECT user_name,add_time,aliww,qq,home_phone,'.
    ...
    $sql_insert = 'INSERT INTO '.$GLOBALS['ecs']->table('order_info').
        ...
        goods_amount,final_amount,shipping_fee,pay_fee,money_paid,add_time,confirm_time,pay_time,shipping_time,
        is_package,inv_no,inv_type,inv_title,add_admin_id,admin_id,group_id,admin_name,team,syn_time,platform,platform_type,operator,
        ...
        money_paid,add_time,confirm_time,pay_time,shipping_time,is_package,inv_no,inv_type,inv_title,add_admin_id,
        admin_id,group_id,admin_name,team,syn_time,platform,platform_type,operator,confirmor,order_type,discount_amount,discount_explain 
    ...
    if ($GLOBALS['db']->query($sql_insert)) {
        ...
        // 记录更新
        record_operate($sql_update, 'order_info');
        ...
        admin_log("订单ID是$order_id",'confirm','ordersyn_verify','order',$order_id);
        die($json->encode($res));
    }
}
...
elseif ($_REQUEST['act'] == 'ordersyn_del')
{
    ...
    if ($GLOBALS['db']->query($sql_update)) {
        ...
        $res['message'] = '订单关闭成功！';
        $res['timeout'] = 2000;
        admin_log("订单ID是$order_id",'del','ordersyn_del','order',$order_id);
...
elseif ($_REQUEST['act'] == 'order_del') {
    ...
    if ($GLOBALS['db']->query($sql_update)) {
        ...
        admin_log("订单ID是$order_id",'cancel','order_del','order',$order_id);
    } else {
        $res['code'] = 0;
...
elseif ($_REQUEST['act'] == 'save')
{
    ...
    $res['req_msg'] = true;
    $res['timeout'] = 2000;
    if (admin_priv('all', '', false) && $_SESSION['admin_id'] != 4) {
    ...
    if ($_REQUEST['info'] == 'shipping')
    {
        ...
        if (admin_priv('shipping_act', '', false) && $_SESSION['admin_id'] != 4){
    ...
    if (isset($_REQUEST['val']))
    {
        ...
        $GLOBALS['db']->query($sql_update);
        if (101 == $_SESSION['admin_id']) {
            file_put_contents('101.txt', $sql_update);
        }

        $sql_select = "SELECT {$_REQUEST['info']} FROM ".$GLOBALS['ecs']->table('ordersyn_info').
            " WHERE order_id=$order_id";
        $res['main'] = $GLOBALS['db']->getOne($sql_select);
        admin_log("订单ID是$order_id",'upd','edit_address','order',$order_id);
    }
...
elseif ($_REQUEST['act'] == 'search_goods')
{
    ...
    $sql_select = 'SELECT CONCAT(g.goods_name,"  【库存：",SUM(s.quantity),"】") goods_name,g.goods_sn goods_id,SUM(s.quantity) quantity FROM '.
        ...
        " s WHERE g.goods_name LIKE '%$keyword%' AND g.is_delete=0 AND g.goods_sn=s.goods_sn GROUP BY s.goods_sn";
    $goods_list = $GLOBALS['db']->getAll($sql_select);
...
elseif ($_REQUEST['act'] == 'add_new_order') {
    ...
    if ($GLOBALS['db']->query($sql_insert))
    {
        ...
        admin_log("订单ID是$order_id",'add_new_order','add','order',$order_id);
...
elseif ($_REQUEST['act'] == 'expressprint')
{
    ...
    $now_time = time();
    $db->query('UPDATE '.$ecs->table('order_info')." SET order_status=5,shipping_status=1,pay_status=2,shipping_time=$now_time,shipping_print_times=shipping_print_times+1 WHERE order_id='$order_id'");
    ...
    if (!isset($_REQUEST['h'])) {
        $smarty->assign('sender', $_SESSION['admin_name']);
        ...
        $assign_region = in_array($order['pro'],array(110000,120000,310000,500000))?$order['zhixiashi']:$order['province'];
        $assign_region = preg_replace('/^黑龙/', '黑龙江', $assign_region);
        $assign_region = preg_replace('/^内蒙/', '内蒙古', $assign_region);
        $smarty->assign('region', $assign_region);
    }
    ...
    $smarty->display('shipping_'.$order['shipping_code'].'.htm');
}
...
elseif ($_REQUEST['act'] == 'info')
{
    ...
    if (isset($_GET['print']))
    {
        ...
        $sql = 'SELECT role_describe FROM '.$ecs->table('role').' r, '.$ecs->table('order_info').
            " i WHERE r.role_id=i.platform AND i.order_id={$order['order_id']}";
        $role = $db->getOne($sql);
        $smarty->assign('role', trim($role));

        $sql = 'UPDATE '.$ecs->table('order_info').' SET order_print_times=order_print_times+1'." WHERE order_id={$order['order_id']}";
        $db->query($sql);
...
elseif ($_REQUEST['act'] == 'shipping_done') {
    ...
    if (!empty($receive_time)) {
        ...
        if ($db->query($sql)) {
            ...
            $sql = 'SELECT IF(shipping_status=2, 1, 0) shipping_status,order_id,user_id FROM '.
                $ecs->table('order_info')." WHERE order_id=$order_id";
            $res = $db->getRow($sql);

            $sql = 'UPDATE '.$ecs->table('users').' u, '.$ecs->table('order_info').' o ';
            if ($res['shipping_status']) {
                $sql .= ' SET u.number_purchased=u.number_purchased+1 ';
            } else {
                $sql .= ' SET u.number_purchased=u.number_purchased-1 ';
            }

            // $sql .= " WHERE u.user_id={$res['user_id']} AND o.order_id={$_POST['id']} ".
            //     ' AND o.add_time>UNIX_TIMESTAMP("2013-02-17 18:00:00")';

            $sql .= " WHERE u.user_id={$res['user_id']} AND o.order_id={$_REQUEST['id']} ".
                ' AND o.add_time>UNIX_TIMESTAMP("2013-02-17 18:00:00")';
            $db->query($sql);
            ...
            // 获取会员部所有客服的user_id及user_name
            $sql_select = 'SELECT user_id FROM '.$ecs->table('admin_user').
                ' WHERE role_id IN ('.MEMBER_SALE.') AND status=1 AND stats=1 AND password<>1 AND assign=1 ORDER BY counter ASC LIMIT 1';
            $admin_id = $db->getOne($sql_select);
...
elseif ($_REQUEST['act'] == 'do_returns') {
    ...
    $sql_select = 'SELECT shipping_status,exp_info FROM '.$GLOBALS['ecs']->table('order_info')." WHERE order_id=$order_id";
    $shipping_info = $GLOBALS['db']->getRow($sql_select);
    if ($shipping_info['shipping_status'] > 2) {
    ...
    if ($GLOBALS['db']->query($sql_insert)) {
        ...
        admin_log("订单ID是$order_id",'apply','do_returns','order',$order_id);
...
elseif ($_REQUEST['act'] == 'rand_order') {
    ...
    if ($_REQUEST['a'] == 'verify') {
        $table = 'ordersyn_info';
        $where_select = ' WHERE order_status=0 AND shipping_status=0 AND admin_id=0 AND platform='.intval($_REQUEST['platform']);
        $where_update = " WHERE order_status=0 AND shipping_status=0 AND operator=0 AND admin_id=0 AND (order_lock IN (0,{$_SESSION['admin_id']}) OR lock_timeout<$now_time) AND platform=".intval($_REQUEST['platform']);
    ...
    $per_admin_order = ceil($order_total/$online_admin_count);
...
elseif ($_REQUEST['act'] == 'batch_print') {
    ...
    // 用一条SQL读取订单数据
    $sql_select = 'SELECT i.order_id,i.consignee,i.tracking_sn,i.mobile,i.pay_id,i.order_type,i.pay_name,i.shipping_name,m.card_number,'.
        'i.admin_id,i.final_amount,r.role_describe platform,i.shipping_fee,i.discount_amount,i.discount_explain,i.address,i.user_id,'.
        'IFNULL(i.order_sn,i.platform_order_sn)order_sn,FROM_UNIXTIME(i.add_time,"%Y-%m-%d")order_time,CONCAT(p.region_name,c.region_name,d.region_name) region FROM '.
        $GLOBALS['ecs']->table('order_info').' i LEFT JOIN '.$GLOBALS['ecs']->table('role').' r ON r.role_id=i.platform LEFT JOIN '.
        $GLOBALS['ecs']->table('region').' p ON p.region_id=i.province LEFT JOIN '.$GLOBALS['ecs']->table('region').
        ' c ON c.region_id=i.city LEFT JOIN '.$GLOBALS['ecs']->table('region').' d ON d.region_id=i.district LEFT JOIN '.
        $GLOBALS['ecs']->table('memship_number').' m ON m.user_id=i.user_id WHERE i.review=1 AND'.
        ' i.order_status=1 AND i.shipping_status=0 ORDER BY  i.goods_kind DESC, i.goods_num ASC';
...
function order_list()
{
    ...
    case 'rand_order' :
        if ($_REQUEST['a'] == 'verify') {
            ...
            $table_user  = 'userssyn';
            $where       = " AND order_status=0 AND o.admin_id=0 AND o.order_lock={$_SESSION['admin_id']} AND o.lock_timeout>$now_time";
            ...
            $order_status = ' AND order_status=1 AND shipping_status=0 AND a.user_id=o.add_admin_id AND s.shipping_id=o.shipping_id '.
                " AND o.order_lock={$_SESSION['admin_id']} AND o.lock_timeout>$now_time";
    ...
    case 'temp_order' : // 新顾客未确认订单
        $table_order = 'ordersyn_info';
        $table_user  = 'userssyn';
        $where       = ' AND order_status=0 AND o.admin_id=0';
        $temp_fields = " ,o.order_lock, IF(lock_timeout<$now_time,'锁定','已锁定') lock_status";
        $sort_by = ' o.add_time ASC ';
        break;
    ...
    case 'order_before_transfer' :
        $table_order = 'order_info';
        $table_user  = 'users';
        $order_status = " AND o.admin_id=a.user_id AND o.order_status=5 AND o.shipping_status IN (1,2) AND u.admin_id={$_SESSION['admin_id']}";
        $table_admin = ','.$GLOBALS['ecs']->table('admin_user').' a ';
        $temp_fields = ',o.review,a.user_name add_admin';
        $sort_by = ' o.shipping_time DESC';
        break;
    ...
    //file_put_contents('inc_files.txt', var_export(get_included_files(), true));
...
function get_order_detail($id, $table)
{
    ...
    if ($table == 'order_info') {
        $sql='SELECT t.*,IFNULL(t.platform_order_sn,t.order_sn) order_sn,p.region_name province,c.region_name city,d.region_name district FROM ';
    } else {
        $sql = 'SELECT t.*, p.region_name province, c.region_name city, d.region_name district FROM ';
    }
    ...
    $sql .= $GLOBALS['ecs']->table($table).' t LEFT JOIN '.$GLOBALS['ecs']->table('region').
        ...
        $GLOBALS['ecs']->table('region').' d ON d.region_id=t.district '." WHERE order_id=$id";
    $order_info = $GLOBALS['db']->getAll($sql);
...
function shipping_synchro($order_id) 
{
    ...
    // 同步标记发货（淘宝）
    if ($order_info['shipping_time'] && in_array($order_info['team'], array(6,21,22))) {
    ...
    if ($order_info['shipping_time'] && $order_info['team'] == 16) {
        ...
        // 获取商品列表
        $sql_select = 'SELECT goods_sn,goods_number FROM '.$GLOBALS['ecs']->table('order_goods')." WHERE order_id=$order_id";
        $goods_list = $GLOBALS['db']->getAll($sql_select);
        unset($val);
        foreach ($goods_list as &$val){
    ...
    if ($order_info['shipping_time'] && $order_info['team'] == 17) {
        ...
        /*
        if (0 && empty($logistics['suning_code'])) {
            $req = new SuningCustomLogisticCompanyGet;

            $req->setCompanyName($logistics['company_name']);
            ...
            $suning->setMethod('suning.custom.logisticcompany.query');
            ...
            $shipping_code = $resp['sn_request']['sn_body']['logisticsCompany']['expressCompanyCode'];
        } else {
 ...
 * @author Nixus
 **/
function verify_price($goods_info)
{
    $mem = new Memcache;
    $mem->connect('127.0.0.1', 11211);

    $right_goods = $mem->get("goods_{$goods_info['goods_sn']}");
    if (empty($right_goods)) {
        $sql_select = 'SELECT goods_name,goods_sn,shop_price,min_price FROM '.$GLOBALS['ecs']->table('goods').
            " WHERE goods_sn='{$goods_info['goods_sn']}'";
    ...
    if ($right_goods['min_price'] <= 0 || $goods_info['goods_price'] > $right_goods['min_price']) {
        return false;
    } else {
        return true;
    }
...
{
    $end_time = time();
    $begin_time = $end_time -3600*15;

    // $platform;
    // $shipping_way;

    // 总记录数
    $sql_select = 'SELECT COUNT(*) FROM '.$GLOBALS['ecs']->table('ordersyn_info').
        " WHERE add_time BETWEEN {$begin_time} AND {$end_time} AND team<>{$_SESSION['role_id']} AND order_status=-1";
    $filter['record_count'] = $GLOBALS['db']->getOne($sql_select);

    // 页码
    $filter['page']      = intval($_REQUEST['page']) ?: 1;

    // 页数
    $filter['page_size'] = intval($_REQUEST['page_size']) ?: 20;

    $page_set = break_pages($record_count, $filter['page_size'], $filter['page']);

    $sql_select = 'SELECT i.consignee,FROM_UNIXTIME(i.add_time,"%Y-%m-%d %H:%i:%s"),i.shipping_name,i.tracking_sn,'.
        'p.region_name prov,c.region_name city,d.region_name dist FROM '.$GLOBALS['ecs']->table('ordersyn_info').
        ' i LEFT JOIN '.$GLOBALS['ecs']->table('region').' p ON i.province=p.region_id LEFT JOIN '.
        $GLOBALS['ecs']->table('region').' c ON c.region_id=i.city LEFT JOIN '.$GLOBALS['ecs']->table('region').
        ' d ON d.region_id=i.district WHERE i.pay_id<>3 AND i.order_status=-1 AND i.add_time BETWEEN '.
        " {$begin_time} AND {$end_time} AND i.team<>{$_SESSION['role_id']} ORDER BY add_time DESC".
        ' LIMIT '.($filter['page']-1)*$filter['page_size'].','.$filter['page_size'];
    $row = $GLOBALS['db']->getAll($sql_select);
