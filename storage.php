...
function stats_shipping_goods () 
{
    ...
    $sql_select = 'SELECT g.goods_sn, SUM(g.goods_number) goods_number, g.goods_name,LEFT(g.goods_name,13) goods_short_name FROM '
    ...
    // 分解套餐 统计套餐中的商品数量
    $sql_select = 'SELECT i.consignee,g.goods_sn, g.goods_number FROM '
        .$GLOBALS['ecs']->table('order_goods')
        .' g LEFT JOIN '.$GLOBALS['ecs']->table('order_info')
        ." i ON g.order_id=i.order_id WHERE $ex_where AND "
        .' i.order_status IN (1,5) AND shipping_status IN (1,2,4) AND g.goods_sn LIKE "%\_%" ';
