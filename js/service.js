...
function searchService(page) {
	...
	Ajax.call('service.php?act=service_search', '&startTime=' + startTime + '&endTime=' + endTime + '&user_id=' + user_id + '&page=' + page, searchServiceResponse, 'POST', 'JSON');
}
...
function search() {
	...
	for (var i = 0; i < filter.length; i++) {
		if (condition == filter[i]) {
			document.forms['searchForm'].elements['search'].style.color = '#fff';
			break;
		}
	}
	...
	if (keyword == '') {
		...
		Ajax.call(url, 'act=search&keyword=' + keyword + '&condition=' + condition, show, 'POST', 'JSON');
	}
}
...
function userInfoRes(res) {
	...
	var insert_div = '<div style="margin:1px 3px;background:#2673C4;color:#FFF;text-align:center;padding:5px;border-radius:3px" id="title_pop">';
