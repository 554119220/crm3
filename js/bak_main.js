...
function switchTab () {
    ...
    Ajax.call(this.href, '', switchTabResponse, 'GET', 'TEXT');
    return false;
}
...
function sendEditInfo (obj)
{
    ...
    var keyword = pattern.exec(obj.value);
    obj.parentNode.id = keyword[1];

    Ajax.call(obj.value, '', showOption, 'GET', 'JSON');
}
...
function addGoods ()
{
    ...
    var data = 'goods_id='+goods_id+'&number='+number+'&price='+price+'&order_id='+id;
...
{
    obj.innerHTML = '';
    obj.innerHTML = '<input type="text" id="trackingSn" name="trackingSn" value="" size="10" onblur="sendTrackingSn(this,' + order_id + ')">';
...
function sendTrackingSn(obj, order_id)
{
    if (obj.value.match(/\d+/))
        Ajax.call('order.php?act=edit_tracking_sn&order_id='+order_id+'&tracking_sn='+obj.value,'',displayTrackingSn,'GET','JSON');
    else 
    {
        var res = new Array ();
        res.errMsg = 3;
...
function showInfo (res)
{
    ...
    obj.innerHTML = res.main+'<button value="'+url+'?act=edit&info='+res.info+'&id='+res.id+'&type='+res.type+'" onclick="sendEditInfo(this)"><img src="images/edit.gif" alt="修改" title="修改"></button>';
...
function addNewGoods ()
{
    ...
    gift.innerHTML = '<img src="images/'+isGift+'.gif" alt="赠品"/><input type="hidden" name="list_gift[]" value="'+isGift+'"/>';
...
function addNewOrder ()
{
    ...
    */
    ...
    if (team != 1 && team != 9 && team != 2 && !platform_order_sn)
    {
...
{
    Ajax.call('report_forms.php', 'act=stats_saler_month&month='+obj.value, querySalesResp, 'POST', 'JSON');
}
...
function pageNextResp (res) {
    ...
    dataMain.tBodies[0].innerHTML = res.main;
...
function statsPreSales () {
    var theForm = document.forms['pageSearch'];
    var data = 'order_month='+theForm.elements['order_month'].value;

    if (theForm.elements['admin_id'] != undefined) {
...
/**
 * 设定当月目标销量
function setSalesTarget (obj) {
    var target = obj.elements['sales_target'].value;
    if (!target) {
        showMsg({req_msg:true,timeout:2000,message:'请输入您的目标销量！'});
        return false;
    }

    if (isNaN(target)) {
        showMsg({req_msg:true,timeout:2000,message:'销量必须是数字！'});
        return false;
    }

    Ajax.call('debris_operation.php', 'act=set_sales_target&sales_target='+target, setSalesTargetResp, 'POST', 'JSON');
    return false;
}

function setSalesTargetResp (res) {
    showMsg(res);
}
*/
