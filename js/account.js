...
function batch(obj)
{
  ...
  Ajax.call('users.php?act=from_to','from_admin='+from_admin+'&to_admin='+to_admin+'&transfer_num='+transfer_num,batchRes,'GET','JSON');
}

function batchRes(res)
{
  showMsg(res);
}
...
function transfer(obj)
{
  ...
  Ajax.call('users.php?act=transfer_submit&','from_phone='+from_phone+'&to_admin='+to_admin,transferRes,'GET','JSON');
}
...
function advanceBatch(obj)
{
  ...
  var active = obj.elements['active'].value;    //顾客疾病
  var eff_id = obj.elements['eff_id'].value;      //顾客类型
  var ser_startTime = obj.elements['ser_startTime'].value;  //服务时间
  var ser_endTime = obj.elements['ser_endTime'].value;
  var add_startTime = obj.elements['add_startTime'].value;
  ...
  {
    Ajax.call('users.php?act=advance_batch&','from_admin='+from_admin+'&actiove='+active+'&eff_id='+eff_id+'&ser_startTime='+ser_startTime+'&ser_endTime='+ser_endTime+'&add_startTime='+add_startTime+'&add_endTime='+add_endTime,advbatchRes,'GET','JSON');  
  }
}
...
//联级选择员工
function getRoleAdminList(role_id)
{
  if(role_id)
  {
    Ajax.call('account_manager.php?act=get_role_admin','role_id='+role_id,getRoleAdminListRes,'GET','JSON');
  }
}

function getRoleAdminListRes(res)
{
  var obj_select = document.getElementById('admin_id');
...
function confirmCheckRes(res)
{
  ...
  if(res.code) {
    ...
    obj_table.deleteRow(res.tr_id);

    //var url = 'users.php?act=user_blacklist';
