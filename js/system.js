...
function admindisablereponse(res)
{
    ...
    showMsg({req_msg:true,timeout:2000,message:msg});
}
...
function validates(obj)
{
    ...
    if ((!home_phone || !!mobile_phone) && !/^1[34578]\d{9}$/.test(mobile_phone)) {
        showMsg({req_msg:true,timeout:2000,message:'请输入正确的手机号码！'});
        return false;
    }

    // 验证用户名
    if (user_name.length <= 0) {
        showMsg({req_msg:true,timeout:2000,message:'请输入（正确的）用户名！'});
        return false;
    }
    ...
    if (province == 0 || city == 0 || address == 0) {
        showMsg({req_msg:true,timeout:2000,message:'请填写详细的省、市、区信息！'});
        return false;
    }
