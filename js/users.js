...
function schBlacklist(obj)
{
  ...
  Ajax.call('users.php?act=user_blacklist','user_name='+user_name+'&phone='+phone+'&operator_in='+operator_in+'&blacklist_status='+blacklist_status+'&from_tab='+'from_tab',schBlacklistRes,'GET','JSON');
}
...
/*主管，组长排查黑名单*/
function checkedBlacklist(user_id,obj){
  var from_table = obj.getAttribute('note');

  if(from_table != '' && user_id !=''){
    Ajax.call('users.php?act=ignore_blacklist','user_id='+user_id+'&from_table='+from_table+'&from_push='+1,checkedBlacklistResp,'GET','JSON');
  }
}

function checkedBlacklistResp(res){
  document.getElementById('pop_ups').innerHTML = '';
