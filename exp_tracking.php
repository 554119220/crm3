...
$start_time = $end_time -10*3600*24;
$where = " AND o.province IN (110000,310000) AND o.shipping_time BETWEEN $start_time AND $end_time ORDER BY o.shipping_time ASC";
$res = $db->query($sql_select.$where); // 发往北京上海的快递信息
$order_list_bjsh = fetch_assoc($res);

$where = " AND o.city=440100 AND o.shipping_time BETWEEN $start_time AND $end_time ORDER BY o.shipping_time ASC";
...
$where = " AND o.shipping_time BETWEEN $start_time AND $end_time ORDER BY o.shipping_time ASC LIMIT 3000";
     ...
     if ($key == '06b6d67cfed750fd')
     {
          ...
          switch ($express_info['state']) {
               ...
               case 4:
                    ...
                    break;
               case 3:
                    $update = "shipping_status=2,receive_time=$nowtime";
                    break;
          }
     ...
     if ($db->query($sql_update) && $status==3)
     {
          ...
          if($is_enable_pur_all)
          {
              ...
              $sql_insert = 'INSERT INTO `crm_integral`'.
                  ...
                  "VALUES({$integral['integral_id']},$points,0,'order',{$order['order_id']},UNIX_TIMESTAMP(NOW()),{$integral['present_end']},0,{$order['user_id']},0,$points,1,{$user['rank_points']},0)";
          ...
          $sql_select = 'SELECT user_id FROM `crm_admin_user` WHERE role_id=9 '.
              ' AND status=1 AND stats=1 AND counter>=0 ORDER BY counter ASC LIMIT 1';
          $admin_id = $db->query($sql_select);
          $admin_id = reset(fetch_assoc($admin_id));
          $admin_id = $admin_id['user_id'];

          $sql_select = 'SELECT user_id FROM `crm_admin_user` WHERE role_id=13';
          ...
          $sql_update = 'UPDATE `crm_users` u,`crm_admin_user` a SET u.admin_name=a.user_name,'.
               "u.admin_id=$admin_id,u.assign_time=".time().',u.role_id=a.role_id,a.counter=a.counter+1 '.
               "WHERE u.admin_id IN ($admin_list) AND u.user_id={$order_result['user_id']} AND a.user_id=$admin_id";
          $db->query($sql_update);
          unset($admin_list);
