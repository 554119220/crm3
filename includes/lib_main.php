 ...
 * @return  void
 */
function set_admin_session($user_id, $username, $role_id, $group_id, $action_list, $last_time)
{
...
function get_customer_type($str = '')
{
    if ($str)
    {
        $sql = 'SELECT type_name FROM '.$GLOBALS['ecs']->table('customer_type')." WHERE available=1 AND type_id IN ($str) ORDER BY sort ASC";
    }
    else
    {
        $sql = 'SELECT * FROM '.$GLOBALS['ecs']->table('customer_type').' WHERE available=1 ORDER BY sort ASC';
    }
    return $GLOBALS['db']->getAll($sql);
}
...
function get_role()
{
    $sql = 'SELECT role_name, role_id FROM '.$GLOBALS['ecs']->table('role').' WHERE role_id NOT IN (3, 4, 5, 8)';
    return $GLOBALS['db']->getAll($sql);
}
...
function get_role_list($type = '')
{
    ...
    $sql  = 'SELECT role_id, role_name, action_list, compete, manager FROM '
        .$GLOBALS['ecs']->table('role').$type.' ORDER BY role_id ASC';
    $list = $GLOBALS['db']->getAll($sql);
