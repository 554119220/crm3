...
function goods_list($is_delete, $real_goods=1)
{
    ...
    if ($result === false)
    {
        ...
        $sql = 'SELECT g.goods_id,g.goods_name,g.goods_type,g.goods_sn,g.shop_price,g.integral,g.shelflife,g.bar_code,g.warn_number,g.is_on_sale,'.
            ' g.goods_weight,IF(SUM(s.quantity), SUM(s.quantity), 0) goods_number, COUNT(s.quantity) stock_times  FROM '.$GLOBALS['ecs']->table('goods').' g LEFT JOIN '.$GLOBALS['ecs']->table('stock_goods').' s ON g.goods_sn=s.goods_sn'.
...
function get_low_stock_goods($filter = 'low_qty')
{
    ...
    if ($filter == 'deadline') {
        ...
        $sql_select = 'SELECT DISTINCT s.goods_sn FROM '.$GLOBALS['ecs']->table('goods').' g,'.$GLOBALS['ecs']->table('stock_goods').
            " s WHERE s.production_day<8*3600*24*30-g.shelflife+$nowtime AND g.is_delete=0 AND s.quantity>0";
...
//分解套餐 
function explain_package($sql,$where){
    // 分解套餐 统计套餐中的商品数量
    $sql_select    = $sql." g.goods_sn LIKE '%\_%' AND $where GROUP BY g.goods_sn";
    ...
    foreach ($package_list as $val){
        $package[$val['goods_sn']]['num'] += $val['num'] * $packing_goods[$val['packing_desc']];
        $package[$val['goods_sn']]['name'] = $package_goods['goods_name'];
    }
