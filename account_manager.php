    ...
    if($user_id)
    {
        ...
        else
        {
            if($result['total'] <= 0)
            {
                ...
                $sql_insert = 'INSERT INTO '.$GLOBALS['ecs']->table('user_blacklist').
                    '(user_id,user_name,admin_id,role_id,operator_in,in_time,type_id,reason)'
                    .$sql_select;    
            } elseif($result['status'] == 1) {
                $sql_insert = 'UPDATE '.$GLOBALS['ecs']->table('user_blacklist').
                    " SET status=0,reason='$reason',type_id=$blacklist_type,".
                    "operator_in={$_SESSION['admin_name']} WHERE user_id=$user_id";
            }
        ...
        if($GLOBALS['db']->getOne($sql_select)) {
            ...
            $sql_insert = 'INSERT INTO '.$GLOBALS['ecs']->table('account_blacklist')
                .'(account_type,blacklist_type,account_value,add_admin,in_time)VALUES('
                ."$account_type,4,'$account_value',"
                .$_SESSION['admin_id'].",UNIX_TIMESTAMP(NOW()))";
