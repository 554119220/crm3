...
elseif ($_REQUEST['act'] == 'signin')
{
    ...
    if(!empty($ec_salt)) {
        /* 检查密码是否正确 */
        $sql = "SELECT user_id,user_name,role_id,group_id,password,last_login,action_list,last_login,suppliers_id,ec_salt"
    ...
    if ($row) {
        ...
        // 登录成功
        set_admin_session($row['user_id'], $row['user_name'], $row['role_id'], $row['group_id'], $row['action_list'], $row['last_login']);
