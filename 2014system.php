...
elseif ($_REQUEST['act'] == 'operator_manage')
{
    ...
    $sql_select = 'SELECT COUNT(*) FROM ' . $GLOBALS['ecs']->table('admin_user');
    ...
    $sql_select = "SELECT ad.*,r.role_name FROM " . $GLOBALS['ecs']->table('admin_user').
        ' AS ad LEFT JOIN '.$GLOBALS['ecs']->table('role').' AS r ON ad.role_id=r.role_id'.
        ' ORDER BY status DESC,add_time DESC LIMIT '.($filter['page']-1)*$filter['page_size'].",{$filter['page_size']}";
...
elseif($_REQUEST['act'] == 'disable')
{
    ...
    //更新状态
    $sql="SELECT status FROM crm_admin_user WHERE user_id=$uid";
    $res = $GLOBALS['db']->getOne($sql);
    if($res==0)
    {
        $sql="UPDATE crm_admin_user SET status=1 WHERE user_id=$uid";
        $res=mysql_query($sql);

        $userjson = array(
            'uid' => intval($uid),
            'content' => '<input type="button" value="在职" class="b_submit" id="posiction_status" onclick="admindisable('.$uid.')">'
        );

        die($json->encode($userjson));

    }
    elseif($res==1)
    {
        $sql="UPDATE crm_admin_user SET status=0 WHERE user_id=$uid";
        $res=mysql_query($sql);
        $userjson = array(
            'uid'=>$_REQUEST['uid'],
            'content' => '<input type="button" style="background:#E0E0E0" class="b_submit" value="离职" onclick="admindisable('.$_REQUEST['uid'].')">');
        die($json->encode($userjson));
    }
    //返回JSON
    //调用表单模板；
...
elseif($_REQUEST['act'] == 'adds') {
    ...
    $sql = 'INSERT INTO '.$GLOBALS['ecs']->table('admin_user').
        '(user_name,password,ec_salt,add_time,status,mobile,role_id,group_id)VALUES'.
        "('$username','$password','$ec_salt','$time',1,'$mobile',$role_id,$group_id)";
...
elseif($_REQUEST['act'] == 'user')
{
    ...
    $sql = "SELECT user_name FROM crm_admin_user WHERE user_name='$user'";
...
{
    $role_id = intval($_REQUEST['role_id']) != 0 ? intval($_REQUEST['role_id']) : 0;
    // 分页
    $filter['page'] = empty($_REQUEST['page']) || (intval($_REQUEST['page'])<=0) ? 1 : intval($_REQUEST['page']);
    if (isset($_REQUEST['page_size']) && intval($_REQUEST['page_size']) > 0)
    {
...
elseif($_REQUEST['act'] == 'issue_notice'){
    $notice_type = mysql_real_escape_string($_REQUEST['notice_type']);
