...
elseif ($_REQUEST['act'] == 'advance_batch')
{
    ...
    $ser_starTime = strtotime($_REQUEST['ser_starTime']);
    ...
    if(!empty($from_admin))
    {
        $where .= " AND u.admin_id=$from_admin ";
        $condition .= "&from_admin=$from_admin";
    }
    ...
    if(!empty($eff_id))
    {
        ...
        $condition .= "&eff_id=$eff_id";
    }

    if(!empty($ser_startTine) && !empty($ser_endTime))
    {
        $where .= " AND u.service_time BETWEEN $ser_startTime AND $ser_endTime ";
        $condition .= "&ser_startTime=$ser_startTime&ser_endTime=$ser_endTime";
    }
    elseif(!empty($ser_startTime) || !empty($ser_endTime))
    {
        if(!empty($ser_endTime))
        {
            $where .= " AND u.service_time=$ser_startTime"; 
            $condition .= "&ser_startTime=$ser_startTine";
        }
        else
        {
            $where .= " AND u.service_time=$ser_endTime"; 
            $condition .= "&ser_endTime=$ser_endTine";
        }
    }

    if(!empty($ser_startTine) && !empty($ser_endTime))
    {
        $where .= " AND u.add_time BETWEEN $add_startTime AND $ser_endTime ";
        $condition .= "&add_startTime=$add_startTime&add_endTime=$add_endTime";
    }
    elseif(!empty($add_endTime) || !empty($add_endTime))
    {
        $where .= " AND u.add_time=$add_endTime"; 
        $condition .= "&add_endTime=$add_endTime";
    }

    $sql_select = ' SELECT COUNT(*) FROM '.$GLOBALS['ecs']->table('users').' AS u '.$where;
    ...
    $sql_select = 'SELECT u.*,a.active_name,e.eff_name,role_name FROM '.$GLOBALS['ecs']->table('users').
        ' AS u LEFT JOIN '.$GLOBALS['ecs']->table('user_active').
        ' AS a ON u.active=a.active_id LEFT JOIN '.
        $GLOBALS['ecs']->table('effects').' AS e ON u.eff_id=e.eff_id '.
        ' LEFT JOIN '.$GLOBALS['ecs']->table('role').
        ' AS r ON r.role_id=u.role_id'.
    ...
    $res['main'] = $smarty->fetch('advance_admin.htm');
...
elseif($_REQUEST['act'] == 'from_to')
{
    ...
    $to_admin_counter = $GLOBALS['db']->getOne(sprintf($sql_select, $to_admin));
     ...
     if (($to_admin_counter + $transfer_num) > $res['max_customer']) {
        $res = array (
            'req_msg' => true,
            'timeout' => 2000,
            'message' => '目标客服顾客数量已达上限或转入后超出数量上限，禁止转入！',
        );
    ...
    $sql = 'UPDATE '.$ecs->table('users')." SET admin_id=$to_admin,admin_name='$to_admin_name',role_id=$to_role_id,".
        "group_id=$to_group_id,last_admin=$from_admin,assign_time=UNIX_TIMESTAMP() WHERE admin_id=$from_admin";
    if ($transfer_num) {
        $sql .= " ORDER BY RAND() LIMIT $transfer_num";
    }
...
elseif ($_REQUEST['act'] == 'find_referrer')
{
    ...
    $sql = 'SELECT DISTINCT user_id,user_name FROM '.$GLOBALS['ecs']->table('users').
        " WHERE (user_name LIKE '%$keywords%' OR mobile_phone LIKE '%$keywords%' OR home_phone LIKE '%$keywords%')";
...
elseif ($_REQUEST['act'] == 'data_users') {
    ...
    $user_list = $mem->get('data_users_'.$_SESSION['admin_id']);

    if (empty($user_list) || !is_array($user_list)) {
        $sql_select = 'SELECT MIN(visits) FROM '.$GLOBALS['ecs']->table('users_data').' WHERE user_type NOT IN (4,6)';
        $min_visits = $GLOBALS['db']->getOne($sql_select);

        // 获取顾客资料
        $sql_select = 'SELECT rec_id,user_name,contact,address,user_type FROM '.$GLOBALS['ecs']->table('users_data').
            " WHERE user_type IN (0,3) AND contact<>'' AND (admin_id=0 OR admin_id={$_SESSION['admin_id']}) AND user_id=0 AND visits=$min_visits LIMIT 100";
        $user_list = $GLOBALS['db']->getAll($sql_select);
        if (empty($user_list)) {
        ...
        $mem->set('data_users_'.$_SESSION['admin_id'], $user_list, 0, 3600);
    }
...
elseif ($_REQUEST['act'] == 'handle_users') {
    ...
    if ($feed == 4) {
        $sql_insert = 'INSERT INTO '.$GLOBALS['ecs']->table('users').
            '(user_name,sex,mobile_phone,home_phone,customer_type,add_time,admin_id,admin_name)VALUES('.
            "'{$req['user_name']}', {$req['sex']}, '{$req['mobile_phone']}', '{$req['home_phone']}', 4, UNIX_TIMESTAMP(),{$_SESSION['admin_id']},'{$_SESSION['admin_name']}')";
        $GLOBALS['db']->query($sql_insert);
        ...
        $res['message'] = '顾客添加成功！顾客分类为意向顾客！';
    } else {
        try {
            $sql_update = 'UPDATE '.$GLOBALS['ecs']->table('users_data').
                " SET visits=visits+1,handle_time=UNIX_TIMESTAMP(),user_type=$feed,admin_id=0 WHERE rec_id=$rec_id";
            $GLOBALS['db']->query($sql_update);
...
elseif($_REQUEST['act'] == 'user_combine'){
    ...
    if($_REQUEST['behave'] == ''){
        ...
        if(admin_priv('all','',false)){
            ...
            $smarty->assign('user_group_view',true);
        }else{
            $power = false;
        }
        ...
        $contact_way = mysql_real_escape_string($_REQUEST['contact_way']);
        $same_item   = mysql_real_escape_string($_REQUEST['same_item']);
        $group_by    = '';
        ...
        if($keyword != ''){
            ...
            }elseif($contact_way == 'name'){
                $where .= " AND u.user_name LIKE '%$keyword%'";
                $group_by = ' GROUP BY u.user_name';
            }else{
                $where .= " AND uc.contact_name='$contact_way' AND uc.contact_value='$keyword' ";
            }
        }elseif($same_item != ''){
            $res['req_msg'] = true;
            $res['message'] = '抱歉，搜索超时';
        ...
        $original_user_id = intval($_REQUEST['original_user_id']);
        $to_user_id       = intval($_REQUEST['to_user_id']);

        if($original_user_id && $to_user_id){

            /*修改顾客联系信息*/
            $sql_update = 'UPDATE '.$GLOBALS['ecs']->table('user_contact').
                " SET user_id=$to_user_id WHERE user_id=$original_user_id"; 
            ...
            if($rank_list){
                ...
                $sql_update = 'UPDATE '.$GLOBALS['ecs']->table('users').
                    " SET user_rank=$rank_id,rank_points=$rank_points WHERE user_id=$to_user_id";
                $result = $GLOBALS['db']->query($sql_update);
            }
            ...
            /*合并服务信息*/
            $sql_update = 'UPDATE '.$GLOBALS['ecs']->table('service').
                " SET user_id=$to_user_id WHERE user_id=$original_user_id";
            $GLOBALS['db']->query($sql_update);

            /*删除顾客*/
            $sql_del = 'DELETE FROM '.$GLOBALS['ecs']->table('users').
...
elseif($_REQUEST['act'] == 'compare_user'){
    ...
    if($user_id_list != ''){
        ...
        $sql_select = 'SELECT user_id,user_name FROM '.$GLOBALS['ecs']->table('users').$where;
...
function user_list()
{
    ...
    if ($result === false) {
        ...
        $ex_where .= ' ORDER by service_time ASC LIMIT '.($filter['page'] -1)*$filter['page_size'].', '.$filter['page_size'];
        $sql .= $ex_where;
...
function get_user_info ($id)
{
    ...
    $sql_select = 'SELECT u.family_id,u.sex,IF(u.calendar=1,CONCAT(u.birthday,"【阴历】"),CONCAT(u.birthday,"【阳历】")) birthday,'.$field.
...
function access_purchase_records ($id)
{
    ...
    $sql_select = 'SELECT o.order_id,IFNULL(o.platform_order_sn, o.order_sn) order_sn,o.consignee,o.order_status,o.shipping_status,o.add_time,'.
...
function outflow_list ($query_field) {
    ...
    $ex_where = " customer_type IN (2, 3, 4, 5, 11) AND u.$query_field NOT BETWEEN ".
        strtotime($filter['start_time']).' AND '.strtotime($filter['end_time']).' AND u.add_time<'.strtotime($filter['start_time']);
...
function referrals_list () {
    ...
    if ($filter['start_time'] && $filter['end_time']) {
        $ex_where .= ' AND u.add_time BETWEEN '.strtotime($filter['start_time'].' 00:00:00').' AND '.strtotime($filter['end_time'].' 23:59:59');
    }
