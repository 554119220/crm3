...
if ($_REQUEST['act'] == 'from_where')
{
    ...
    $sql = 'SELECT final_amount, order_source FROM '.$ecs->table('order_info').' WHERE order_source>0 AND add_time>UNIX_TIMESTAMP("'.$date.'") -6*3600';
    $_POST['end_date'] && $sql = 'SELECT final_amount, order_source FROM '.$ecs->table('order_info').' WHERE order_source>0 AND add_time<UNIX_TIMESTAMP("'.$_POST['end_date'].'") +18*3600';

    $_POST['start_date'] && $_POST['end_date'] && $sql = 'SELECT final_amount, order_source FROM '.$ecs->table('order_info').' WHERE order_source>0 AND add_time>UNIX_TIMESTAMP("'.$_POST['start_date'].'") -6*3600 AND add_time<UNIX_TIMESTAMP("'.$_POST['end_date'].'") +18*3600';
...
elseif($_REQUEST['act'] == 'sales_volume')
{
    ...
    if (admin_priv('sales_report', '', false) === false)
    {
        ...
        if (array_key_exists($_SESSION['admin_id'], $saler))
        {
            ...
            $sale['month'] = $GLOBALS['db']->getRow($sql);  // 昨天销量 
...
elseif ($_REQUEST['act'] == 'forecast')
{
    ...
    if ($take || $time)
    {
        if ($time != 'over')
        {
            $sql = "SELECT order_id, user_id, consignee, tel, mobile, goods_name, goods_number, FROM_UNIXTIME(shipping_time+take_days*goods_number/$take, '%m-%d') coming, user_name FROM ".$ecs->table('forecast_view')." WHERE shipping_time+take_days*goods_number/$take>$start AND shipping_time+take_days*goods_number/$take<$end AND shipping_time+7*3600*24>service_time AND service_time<$start";
            $sql .= $where;
            $res = $db->getAll($sql);

            $smarty->assign('user_list', $res);
            $cont = $smarty->fetch('forecast.htm');
            die($cont);
        }
        else
        {
            $sql = "SELECT order_id, user_id, consignee, tel, mobile, goods_name, goods_number, FROM_UNIXTIME(shipping_time+take_days*goods_number/$take, '%m-%d') coming, user_name FROM ".$ecs->table('forecast_view')." WHERE (shipping_time+take_days*goods_number/$take)<$start AND shipping_time+7*3600*24>service_time AND service_time<$start";
            $sql .= $where;
            $res = $db->getAll($sql);
    ...
    {
        $sql = 'SELECT order_id, user_id, consignee, tel, mobile, goods_name, goods_number, FROM_UNIXTIME(shipping_time+take_days*goods_number, "%m-%d") coming, user_name FROM '.$ecs->table('forecast_view')." WHERE shipping_time+take_days*goods_number>$start AND shipping_time+take_days*goods_number<$end AND shipping_time+7*3600*24>service_time AND service_time<$start";
        $sql .= $where;
    }
    $res = $db->getAll($sql);

    $smarty->assign('forecast', 1);
    $smarty->assign('user_list', $res);

    $smarty->display('forecast.htm');
}
elseif ($_REQUEST['act'] == 'rebuy')
{
    ...
    $sql = 'SELECT user_id, consignee, final_amount, admin_name, admin_id, FROM_UNIXTIME(add_time, "%Y-%m-%d") add_time FROM '
        ...
        " WHERE order_status=5 AND shipping_status>0 AND pay_status=2 AND final_amount>0 AND 
        user_id IN ($uid) $team $admin_id AND add_time>$start AND add_time<$end ORDER BY add_time DESC";
